...
async function getTaskCategories(req, res, next) {
  try {
    ...
    let sql = 'SELECT * FROM task_categories WHERE user_id = ?';
    const params = [userId];
...
async function createTaskCategory(req, res, next) {
  try {
    ...
    const sql = `INSERT INTO task_categories 
...
async function updateTaskCategory(req, res, next) {
  try {
    const { id } = req.params;
    ...
    const sql = `UPDATE task_categories SET
      name = COALESCE(?, name),
      icon = COALESCE(?, icon),
...
async function deleteTaskCategory(req, res, next) {
  try {
    const { id } = req.params;
    ...
    const sql = 'UPDATE task_categories SET status = 0 WHERE id = ? AND user_id = ?';
